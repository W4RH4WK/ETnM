...
architectures (x86\_64 and ARM) is taken prior concluding this write-up. Some
...
no special mitigation mechanisms in place denying even the simplest exploits.
This is the point in time where we will start off. First we talk about two very
simple exploits, namely the format string exploit and the buffer overflow in
combination with shell code. Although there is a huge collection of
...
executable itself with its \texttt{.text} and \texttt{.data} segment, the heap
...
one may already derive from the name, this is about submitting more data to a
...
\begin{figure}[H]
    ...
    \caption{Stack frame containing a buffer~\cite{wiki:buffer_overflow}}
...
But since this is pretty straight forward let's go beyond that and see what
...
A more comprehensive article \cite{phrack_stack_smash} about Stack Smashing is
available on \texttt{phrack}\footnote{\url{http://phrack.org/}}.
...
Generally functions handling strings will terminate upon reading a
\texttt{NULL} byte. Offensive Security's \textit{Metasploit Fundamentals} has a
dedicated
section\footnote{\url{https://www.offensive-security.com/metasploit-unleashed/generating-payloads/}
on February 2016} about generating payloads (utilizing Metasploit) without
unwanted bytes. One could work around this manually by setting the first part
of register prior setting the second part. Another way would be by issuing an
\texttt{XOR} with the register itself as in the first instruction of the
example payload.
...
by hitting return a few times. You can enter commands and receive output as
usual.
...
executing injected code as we did in the previous shell code example. This
...
\begin{listing}[h!]
    \codefile[escapeinside=\%\%]{../src/shell_code/vuln_readelf.txt}
...
The first example shown in \cref{sec:buffer_overflow} already illustrated the
power that comes along with controlling the return address. It enabled us to
jump to a completely different function upon execution. The target binary may
not contain a function doing exactly what an attacker wants to do. But by
...
The final \texttt{ret} statement of a gadget is required for chaining them
...
of a list of return addresses (the start of each gadget) interleaved by some
padding. Each provided address will be consumed one by one at the end of each
gadget to \emph{jump} to the next one. The payload may still contain data such
as \texttt{"/bin//sh"} if necessary.
...
observed. Of course each gadget can be arbitrarily long, we just used a length
of 5 instructions in this example.

Various tools simplifying the process of finding gadgets (and even whole
chains) already exist, for instance
\texttt{ROPgadget.py}\footnote{\url{https://github.com/JonathanSalwan/ROPgadget}},
but they go beyond the scope of this writing~\cite{shacham2007}.
...
layout is randomized so an attacker cannot reliably use ROP. An attacker will
...
randomize these segments too, but one would have to compile every relevant
package again instead of using the distribution vendor's pre-compiled binary.
...
without ASLR. But before exploiting this fact, have a look at the randomized
addresses again.
...
\begin{quote}
    ...
    \[ \texttt{0xb7e72280} - \texttt{0xd0f0} = \texttt{0xb7e65190} \]
\end{quote}
...
offset, we leave this as an exercise for the reader.
...
CFI is a big topic and, similar to other topics already mentioned, goes beyond
the scope of this write-up. The first pointer we hand the reader aims at the
corresponding
...
techniques such as \textit{heap spraying} are basically useless, but we can
still resort to the info leak. Breaking a canary via brute force gets only a
little bit harder, but this has already been shown in an example.
...
beyond simple manipulations like changing local variables, we craft shell code
...
broken with brute force (including an unexpected low upper bound) in certain
scenarios.
